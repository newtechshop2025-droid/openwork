...
export function ReactSessionComposer(props: ComposerProps) {
  ...
  return (
    <div
      ref={rootRef}
      className={`sticky bottom-0 ${toolMenuOpen ? "z-50" : "z-20"} bg-gradient-to-t from-dls-surface via-dls-surface/95 to-transparent px-2 md:px-4 pb-0.5 ${props.compactTopSpacing ? "pt-0" : "pt-0.5"}`}
    ...
    >
      <div className="max-w-[960px] mx-auto">
          ...
          <div className="px-4 pt-3 pb-2">
            ...
            <div className="mt-2 flex items-end justify-between gap-2">
              <div className="flex items-center gap-1.5">
                ...
                <button
                  type="button"
                  className={`inline-flex h-9 max-h-9 w-9 items-center justify-center rounded-md text-gray-10 transition-colors hover:bg-gray-3 ${!props.attachmentsEnabled ? "cursor-not-allowed opacity-60" : ""
                    }`}
                  onClick={() => {
                ...
                <div className="h-4 w-px bg-dls-border mx-1 self-center" />
                <ModelSelect
              ...
              <div className="ml-auto flex shrink-0 items-end gap-1.5">
                {props.busy ? (
                  <>
                    ...
                    <div className={`flex items-end gap-1.5 ${followupShake ? "animate-shake" : ""}`}>
                      <button
                        ...
                        disabled={!canSend}
                        className={`inline-flex h-9 max-h-9 items-center gap-2 rounded-full px-4 text-[13px] font-medium transition-colors ${canSend
                      ...
                      <button
                        ...
                        disabled={!canSend}
                        className={`relative inline-flex h-9 max-h-9 items-center gap-2 rounded-full px-4 text-[13px] font-medium transition-colors ${canSend
                  ...
                  <button
                    ...
                    className={`inline-flex h-9 max-h-9 items-center gap-2 rounded-full px-4 text-[13px] font-medium transition-colors ${!canSend || props.disabled
                        ? "bg-gray-4 text-gray-10"
                        : "bg-[var(--dls-accent)] text-[var(--dls-accent-fg)] hover:bg-[var(--dls-accent-hover)]"
                      }`}
        ...
        {/* Old below-panel control strip removed */}
      </div>
    </div>
  );
}
